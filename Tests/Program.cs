                        ...
                        {
                            {
                                while (oldIp.ToString() == ip.ToString())
                                {
                                    ...
                                    if (index != -1)
                                        IPAddress.TryParse(nodes.FirstOrDefault().InnerText.Substring(0, index), out ip);
                                    else
                                    {
                            ...
                            try
                            {
                                ...
                                foreach (var s in stringArray)
                                {
                                    if (s.Contains("BUILT"))
                                    {
                                        ...
                                        server.Send(Encoding.ASCII.GetBytes($"closecircuit {circuit}" + Environment.NewLine));
                                        data = new byte[1024];
                                        receivedDataLength = server.Receive(data);
                                        stringData = Encoding.ASCII.GetString(data, 0, receivedDataLength);
                                    }
